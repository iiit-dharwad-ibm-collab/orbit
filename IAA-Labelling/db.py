...
# ---------------------------------------------------------------------------
# Human-vs-judge response grading (separate from the dataset-instance flow above)
# ---------------------------------------------------------------------------

def init_response_db() -> None:
    """Create the response-grading table (model responses scored 1-5 by humans)."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            """
...
def save_response_score(response_id: str, annotator: str, score: int, notes: str = "") -> None:
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            """
            INSERT INTO response_scores (response_id, annotator, score, notes)
            VALUES (%s, %s, %s, %s)
            ON CONFLICT (response_id, annotator) DO UPDATE SET
                score = EXCLUDED.score,
                notes = EXCLUDED.notes,
                created_at = NOW()
            """,
            (response_id, annotator.strip(), int(score), notes),
        )


def fetch_response_scores(annotator: str):
    """Return {response_id: {'score': int, 'notes': str}} already saved by this annotator."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT response_id, score, notes FROM response_scores WHERE annotator = %s",
            (annotator.strip(),),
        )
        return {rid: {"score": s, "notes": n} for rid, s, n in cur.fetchall()}


def fetch_response_score_summary():
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            """
            SELECT annotator, COUNT(*) AS total, ROUND(AVG(score)::numeric, 2) AS mean_score
            FROM response_scores GROUP BY annotator ORDER BY annotator
            """
        )
        return cur.fetchall()


def export_response_scores():
    """All scores, for computing human-vs-judge agreement offline."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT response_id, annotator, score, notes, created_at "
            "FROM response_scores ORDER BY response_id, annotator"
        )
        return cur.fetchall()
